    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if($shippingtitle=='Expedited Shipping'){
                      ...
                      foreach ($_items as $_item){ ?>
                        <?php //if (!$_item->getParentItem()) : ?>
                            ...
                            if($childProd = current($_item->getChildrenItems())){
                                $image_url = $_imagehelper->init($childProd->getProduct(), 'category_page_list', array('height' => 156 , 'width'=> 170))->getUrl();
                                $prodid=$childProd->getProduct()->getId();
                                $product = $objectManager->create('Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable')->getParentIdsByChild($prodid);
                                // echo $product[0];
                                if(isset($product[0])){
                                    $product = $objectManager->create('Magento\Catalog\Model\Product')->load($product[0]);
                                    $image_url = $_imagehelper->init($product, 'product_page_image_small')->setImageFile($product->getImage())->resize(156, 170)->getUrl();
                                }
                            }
                            else{
                                $image_url = $_imagehelper->init($_item->getProduct(), 'category_page_list', array('height' => 156 , 'width'=> 170))->getUrl();
                            }
                          ...
                          <td
                            ...
                            <p style="margin-top:0;margin-bottom:10px">SKU: '.$_item->getSku().'</p>';
                                  if (isset($options['options']) && !empty($options['options'])) { 
                                      $html.='<dl style="margin-top:0;margin-bottom:0;padding-left:0">
                                    ...
                                    $html.='<dl style="margin-top:0;margin-bottom:0;padding-left:0">
                                      <dt style="font-weight:700;margin-top:0;margin-bottom:0;padding-bottom:0"><strong
                                          style="font-weight:700"><em style="font-style:italic">Size</em></strong></dt><dd style="margin-top:0;margin-left:0;margin-bottom:0;padding-bottom:0;padding-left:0">'; 
                                    $product = $_item->getProduct();
                                    $size = $product->getSize(); // get size value
                                    if($product->getSize()) {
                                            $html.= $product->getAttributeText('size'). '<br />' . '<br />'; // get size label  
                                    }
                                    $html .=' </dd>
                                            </dl>';
                                }
                            $html .='
                            Qty:('.number_format($_item->getQtyOrdered(), 0, '.', '').')
                          </td>
                          <td
                            style="vertical-align:top;padding:10px;border-top:1px solid #d1d1d1;text-align:right">
                            <span>$'.number_format($_item->getPrice(), 0, '.', '').'</span> </td>
                        </tr>
                      ';
                    ...
                    $html.=' </tbody> <tfoot>
                        <tr>
                          ...
                          <td
                            ...
                            <span style="white-space:nowrap">$'.number_format($order->getSubtotal(),0,'.','').'</span> </td>
                        </tr>
                        <tr>
                          <th colspan="2" scope="row"
                          ...
                          <td
                            ...
                            <span style="white-space:nowrap">$'.number_format($order->getShippingAmount(), 0, '.', '').'</span> </td>
                        </tr>
                        <tr>
                          <th colspan="2" scope="row"
                            style="vertical-align:top;background-color:#f2f2f2;border-top:0;font-weight:400;padding:10px;text-align:right;padding-top:0">
                          ...
                          <td
                            style="vertical-align:top;background-color:#f2f2f2;border-top:0;padding:10px;text-align:right;padding-top:0">
                            <strong style="font-weight:700"><span style="white-space:nowrap">$'.number_format($order->getGrandTotal(), 0, '.', '').'</span></strong> </td>
                        </tr>
                      </tfoot>
                    </table>
                  </td>
            ...
            $transport = $this->transportBuilder
                ...
                ->addTo($adminEmail)
                ->addCc(['info@jacarandaliving.com','nitin.sharma@purplecommerce.com'])
                ->getTransport();
            $message = '';
